...
/// Copied from Crest-Examples. Thanks to Crest team for making it available.
/// </summary>
...
#pragma warning restore 414
    ...
    bool _inputSystem = true;
    readonly float _keyDebounceTime = 1.0f; // seconds
    float _lastKeyTime = -Mathf.Infinity;
...
#pragma warning restore CS0108
    ...
    void Update()
    {
        ...
        {
            if (Time.time - _lastKeyTime > _keyDebounceTime)
            {
                _inputSystem = !_inputSystem;
                Debug.Log($"[{transform.name}] Keyboard control toggled to {_inputSystem}");
